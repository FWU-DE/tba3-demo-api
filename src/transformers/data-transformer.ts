...
export class DataTransformer {
  ...
  private generateComparisonData(data: StatisticItem[]): ComparisonData[] {
    ...
    return data.map(item => {
      const baseline = baselines[item.label as keyof typeof baselines] || { state_avg: 3.0, national_avg: 2.9, percentile_variance: 0.15 };
      ...
      if (performance_ratio > 1.2) percentile_rank = 75 + Math.random() * 20; // 75-95th percentile
      else if (performance_ratio > 1.1) percentile_rank = 65 + Math.random() * 15; // 65-80th percentile
      else if (performance_ratio > 0.9) percentile_rank = 40 + Math.random() * 25; // 40-65th percentile
      else if (performance_ratio > 0.8) percentile_rank = 25 + Math.random() * 20; // 25-45th percentile
      else percentile_rank = 10 + Math.random() * 20; // 10-30th percentile
